...
<pattern>
    ...
    <pattern>
        <rule context="//tei:rs[@type='building']">
            <assert test="@subtype=('temple','palace','monument', 'fort', 'house', 'tower', 'prison', 'wall')">Unacceptable @subtype value (or missing @subtype value) for rs[@type="building"]! </assert>
        </rule>
    </pattern> 
    <pattern>
        <rule context="//tei:rs[@type='earthworks']">
            <assert test="@subtype=('garden','mine','lumber', 'well', 'irrig', 'grave')">Unacceptable @subtype value (or missing @subtype value) for rs[@type="earthworks"]! </assert>
        </rule>
    </pattern> 
    <pattern>
        <rule context="//tei:rs[@type='metaplace']">
            <assert test="@subtype=('garden','barrier', 'simil', 'divin', 'under', 'celest', 'passage')">Unacceptable @subtype value (or missing @subtype value) for rs[@type="metaplace"]! </assert>
        </rule>
    </pattern> 
    <pattern>
